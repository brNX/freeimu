...
#define TO_READ (6)      // num of unsigned chars we are going to read each time (two unsigned chars for each axis)
...
void ADXL345::readAccel(int *x, int *y, int *z)
{
	...
	// each axis reading comes in 10 bit resolution, ie 2 unsigned chars.  Least Significat unsigned char first!!
	// thus we are converting both unsigned chars in to one int
...
void ADXL345::writeTo(unsigned char address, unsigned char val)
{
	...
	i2cWrite(_dev_address, tx, 2);
}

// Reads num unsigned chars starting from address register on device in to _buff array
void ADXL345::readFrom(unsigned char address, int num, unsigned char _buff[])
{
	...
	i2cWrite(_dev_address, tx, 1);
	i = i2cRead(_dev_address, _buff, num);
...
void ADXL345::getRangeSetting(unsigned char* rangeSetting)
{
	unsigned char _b;
...
// Sets the THRESH_TAP unsigned char value
...
void ADXL345::setTapThreshold(int tapThreshold)
{
	...
	unsigned char _b = (unsigned char) tapThreshold;
	writeTo(ADXL345_THRESH_TAP, _b);
}

// Gets the THRESH_TAP unsigned char value
...
// Sets the OFSX, OFSY and OFSZ unsigned chars
// OFSX, OFSY and OFSZ are user offset adjustments in twos complement format with
// a scale factor of 15,6mg/LSB
// OFSX, OFSY and OFSZ should be comprised between
void ADXL345::setAxisOffset(int x, int y, int z)
{
	writeTo(ADXL345_OFSX, (unsigned char)x);
	writeTo(ADXL345_OFSY, (unsigned char) y);
	writeTo(ADXL345_OFSZ, (unsigned char) z);
}

// Gets the OFSX, OFSY and OFSZ unsigned chars
...
// Sets the DUR unsigned char
// The DUR unsigned char contains an unsigned time value representing the maximum time
...
void ADXL345::setTapDuration(int tapDuration)
{
	...
	unsigned char _b = (unsigned char) tapDuration;
	writeTo(ADXL345_DUR, _b);
}

// Gets the DUR unsigned char
...
{
	unsigned char _b = (unsigned char) floatTapLatency;
...
void ADXL345::setDoubleTapWindow(int floatTapWindow)
{
	...
	unsigned char _b = (unsigned char) floatTapWindow;
...
// Sets the THRESH_ACT unsigned char which holds the threshold value for detecting activity.
...
void ADXL345::setActivityThreshold(int activityThreshold)
{
	...
	unsigned char _b = (unsigned char) activityThreshold;
	writeTo(ADXL345_THRESH_ACT, _b);
}

// Gets the THRESH_ACT unsigned char
...
// Sets the THRESH_INACT unsigned char which holds the threshold value for detecting inactivity.
...
void ADXL345::setInactivityThreshold(int inactivityThreshold)
{
	...
	unsigned char _b = (unsigned char) inactivityThreshold;
	writeTo(ADXL345_THRESH_INACT, _b);
}

// Gets the THRESH_INACT unsigned char
...
void ADXL345::setTimeInactivity(int timeInactivity)
{
	...
	unsigned char _b = (unsigned char) timeInactivity;
...
void ADXL345::setFreeFallThreshold(int freeFallThreshold)
{
	...
	unsigned char _b = (unsigned char) freeFallThreshold;
...
void ADXL345::setFreeFallDuration(int freeFallDuration)
{
	...
	unsigned char _b = (unsigned char) freeFallDuration;
...
void ADXL345::setRate(float rate)
{
	...
	if (r <= 9) {
		readFrom(ADXL345_BW_RATE, 1, &_b);
		_s = (unsigned char) (r + 6) | (_b & 0b11110000);
		writeTo(ADXL345_BW_RATE, _s);
	}
}

void ADXL345::set_bw(unsigned char bw_code)
{
...
unsigned char ADXL345::get_bw_code()
{
...
bool ADXL345::getInterruptSource(unsigned char interruptBit)
{
	return getRegisterBit(ADXL345_INT_SOURCE,interruptBit);
}

bool ADXL345::getInterruptMapping(unsigned char interruptBit)
{
...
void ADXL345::setInterruptMapping(unsigned char interruptBit, bool interruptPin)
{
	setRegisterBit(ADXL345_INT_MAP, interruptBit, interruptPin);
}

bool ADXL345::isInterruptEnabled(unsigned char interruptBit)
{
	return getRegisterBit(ADXL345_INT_ENABLE,interruptBit);
}

void ADXL345::setInterrupt(unsigned char interruptBit, bool state)
{
	setRegisterBit(ADXL345_INT_ENABLE, interruptBit, state);
}

void ADXL345::setRegisterBit(unsigned char regAdress, int bitPos, bool state)
{
	unsigned char _b;
...
bool ADXL345::getRegisterBit(unsigned char regAdress, int bitPos)
{
	unsigned char _b;
	readFrom(regAdress, 1, &_b);
	return ((_b >> bitPos) & 1);
