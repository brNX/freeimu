...
public:
  ...
  unsigned char error_code;       // Initial state
  ...
  void set_bw(unsigned char bw_code);
  unsigned char get_bw_code();

  unsigned char getInterruptSource();
  bool getInterruptSource(unsigned char interruptBit);
  bool getInterruptMapping(unsigned char interruptBit);
  void setInterruptMapping(unsigned char interruptBit, bool interruptPin);
  bool isInterruptEnabled(unsigned char interruptBit);
  void setInterrupt(unsigned char interruptBit, bool state);

  void getRangeSetting(unsigned char* rangeSetting);
...
private:
  void readFrom(unsigned char address, int num, unsigned char*);
  void setRegisterBit(unsigned char regAdress, int bitPos, bool state);
  bool getRegisterBit(unsigned char regAdress, int bitPos);
  unsigned char _buff[6] ;    //6 unsigned chars buffer for saving data read from the device
  int _dev_address;
};
