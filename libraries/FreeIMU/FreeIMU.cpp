...
extern int decimalOf(float val);
...
void FreeIMU::calLoad()
{
	...
	EEPROMRead(&pulRead, 0x0, sizeof(unsigned long));

  if(pulRead == FREEIMU_EEPROM_SIGNATURE) { // check if signature is ok so we have good data

  	EEPROMRead(&pulRead, 4, sizeof(unsigned long));
  	acc_off_x = pulRead;
  	EEPROMRead(&pulRead, 8, sizeof(unsigned long));
  	acc_off_y = pulRead;
  	EEPROMRead(&pulRead, 12, sizeof(unsigned long));
  	acc_off_z = pulRead;

  	EEPROMRead(&pulRead, 16, sizeof(unsigned long));
  	magn_off_x = pulRead;
  	EEPROMRead(&pulRead, 20, sizeof(unsigned long));
  	magn_off_y = pulRead;
  	EEPROMRead(&pulRead, 24, sizeof(unsigned long));
  	magn_off_z = pulRead;

  	EEPROMRead((unsigned long*)&acc_scale_x, 28, sizeof(unsigned long));
  	EEPROMRead((unsigned long*)&acc_scale_y, 32, sizeof(unsigned long));
  	EEPROMRead((unsigned long*)&acc_scale_z, 36, sizeof(unsigned long));

  	EEPROMRead((unsigned long*)&magn_scale_x, 40, sizeof(unsigned long));
  	EEPROMRead((unsigned long*)&magn_scale_y, 44, sizeof(unsigned long));
  	EEPROMRead((unsigned long*)&magn_scale_z, 48, sizeof(unsigned long));
...
//  UARTprintf("mcounter=%d\n", miliCounter);
  lastUpdate = now;
//  for (int i=0; i<9; i++)
//  	UARTprintf("%d.%04d ", (int)val[i], decimalOf(val[i]));
//  UARTprintf("\n");
...
float FreeIMU::getBaroAlt(float sea_press)
{
  ...
  //UARTprintf("temp=%d pres=%d\n", (int)temp, (int)press);
  return ((powf((sea_press / press), 1/5.257) - 1.0) * (temp + 273.15)) / 0.0065;
}
