...
#define TEMP_OUT           0x1B  // R	SENSOR: Temperature 2unsigned chars
#define GYRO_XOUT          0x1D  // R	SENSOR: Gyro X 2unsigned chars
#define GYRO_YOUT          0x1F  // R	SENSOR: Gyro Y 2unsigned chars
#define GYRO_ZOUT          0x21  // R	SENSOR: Gyro Z 2unsigned chars
...
public:
  ...
  void init(unsigned int address, unsigned char _SRateDiv, unsigned char _Range, unsigned char _filterBW, unsigned char _ClockSrc, bool _ITGReady, bool _INTRawDataReady);
    
  // Who Am I
  unsigned char getDevAddr();
  void setDevAddr(unsigned int _addr);
  // Sample Rate Divider
  unsigned char getSampleRateDiv();
  void setSampleRateDiv(unsigned char _SampleRate);
  // Digital Low Pass Filter BandWidth and SampleRate 
  unsigned char getFSRange();
  void setFSRange(unsigned char _Range); // RANGE2000
  unsigned char getFilterBW();
  void setFilterBW(unsigned char _BW); // see register parameters above
  ...
  void setClockSource(unsigned char _CLKsource); // see register parameters above
