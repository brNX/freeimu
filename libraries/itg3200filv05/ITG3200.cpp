...
void ITG3200::init(unsigned int address, unsigned char _SRateDiv, unsigned char _Range, unsigned char _filterBW, unsigned char _ClockSrc, bool _ITGReady, bool _INTRawDataReady)
{
	...
	delayMSec(GYROSTART_UP_DELAY);  // startup
}

unsigned char ITG3200::getDevAddr()
{
...
unsigned char ITG3200::getSampleRateDiv()
{
	readmem(SMPLRT_DIV, 1, &_buff[0]);
	return _buff[0];
}

void ITG3200::setSampleRateDiv(unsigned char _SampleRate)
{
	writemem(SMPLRT_DIV, _SampleRate);
}

unsigned char ITG3200::getFSRange()
{
	readmem(DLPF_FS, 1, &_buff[0]);
	return ((_buff[0] & DLPFFS_FS_SEL) >> 3);
}

void ITG3200::setFSRange(unsigned char _Range)
{
	readmem(DLPF_FS, 1, &_buff[0]);
	writemem(DLPF_FS, ((_buff[0] & ~DLPFFS_FS_SEL) | (_Range << 3)) );
}

unsigned char ITG3200::getFilterBW()
{
	readmem(DLPF_FS, 1, &_buff[0]);
	return (_buff[0] & DLPFFS_DLPF_CFG);
}

void ITG3200::setFilterBW(unsigned char _BW)
{
	readmem(DLPF_FS, 1, &_buff[0]);
...
void ITG3200::reset()
{
	...
	delayMSec(GYROSTART_UP_DELAY); //gyro startup
}
...
unsigned char ITG3200::getClockSource()
{
	readmem(PWR_MGM, 1, &_buff[0]);
	return (_buff[0] & PWRMGM_CLK_SEL);
}

void ITG3200::setClockSource(unsigned char _CLKsource)
{
	readmem(PWR_MGM, 1, &_buff[0]);
...
void ITG3200::readmem(uint8_t _addr, uint8_t _nbytes, uint8_t __buff[])
{
  ...
  delayMSec(1);
  i2cRead(_dev_address, __buff, _nbytes);
